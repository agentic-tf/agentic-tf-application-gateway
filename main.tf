...
resource "azurerm_application_gateway" "this" {
  ...
  gateway_ip_configuration { name = "gw-ip-config"; subnet_id = var.subnet_id }
  frontend_port { name = "http"; port = 80 }
  frontend_ip_configuration { name = "frontend"; public_ip_address_id = var.public_ip_id }
  ...
  request_routing_rule { name = "rule"; rule_type = "Basic"; http_listener_name = "listener"; backend_address_pool_name = "backend-pool"; backend_http_settings_name = "http-settings"; priority = 100 }
  tags = var.tags
}
